...
Message loop 示例展示了 Runtime 如何根据真实的控制流访问和展开后的命令调用
逐步构建流程图。Trace 事件到达时节点会依次出现，当前语句或调用会获得明显高亮
并被画布自动跟随；选中 `lark-cli` 后，可以在节点检查器中查看该节点的具体
Invocation 和执行结果。
...
- **Runtime** 是默认视角。它按照执行顺序展示实际访问的循环、条件以及展开后的
  命令调用，包括由命令替换、`eval` 或 `source` 产生的命令。展开过程的容器节点
  不会显示，因此一个解码后的调用会表现为“生成该调用的命令”，随后连接到“最终
  执行的命令”。循环头和内部条件每次真实重入都会重新出现并高亮。未
...
- AST 节点每次被真实控制流访问时都会重新高亮，因此循环头和循环体语句会在每轮
  ...
  每次控制流访问和每次具体调用分别记录为独立 occurrence。
