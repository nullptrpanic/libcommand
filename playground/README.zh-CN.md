...
Message loop 示例展示了 Runtime 如何根据实际展开后的命令调用逐步构建流程图。
Trace 事件到达时节点会依次出现，当前调用会获得明显高亮并被画布自动跟随；选中
`lark-cli` 后，可以在节点检查器中查看该节点的具体 Invocation 和执行结果。

![Playground 实时构建 Runtime 命令图并查看一个 lark-cli 节点的结果](../assets/playground/runtime-flow.gif)
...
的分支体和循环体仍然展示；`eval` 解析新源码后，其顶层 `lark-cli` 语法会挂到
同一棵 AST 上。
...
- **Runtime** 是默认视角。它按照执行顺序展示实际展开后的命令调用，包括由命令
  替换、`eval` 或 `source` 产生的命令。展开过程的容器节点不会显示，因此一个
  解码后的调用会表现为“生成该调用的命令”，随后连接到“最终执行的命令”。未
...
- 模拟执行前即可查看 **AST**。一次只解析的 Worker 请求会构建当前源码的静态
  语法骨架，但不会执行 Shell 代码或命令 Handler。模拟执行会在原节点上更新
  状态，并把新发现的 `eval` 和 `source` 动态语法挂到现有树上。AST 会折叠 Trace
  中标记为 `Embedded` 的条件求值、命令替换和管道操作数，因为父节点已经表达了
  它们；这些节点仍完整保留在 Trace 和 Runtime 视图中。从未被任何路径执行的
  语句仍然保留，便于对比语法结构和可达性。
- 实线节点表示模拟器已经执行过该节点。
- 紫色分叉表示因为某个值或状态 unresolved 而探索出的不同路径。
- 虚线节点表示解析器发现了对应语法，但没有模拟路径执行到该节点。
- 循环中的重复执行会累计到同一个 AST 节点；Runtime 仍会分别展示每次具体调用。
- AST 初始位于顶部中央。顺序语句向下推进，互斥分支处在同一水平线，并在后续
  语句前汇合。没有穷尽分支的条件语句会保留一条直接后继边；循环体内部仍严格
  按源码顺序连接。实时跟随会让当前节点保持水平居中，纵向仅滚动到足以让节点
  及其上下文保持可见的位置。
